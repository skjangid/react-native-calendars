...
class ReservationList extends Component {
  ...
  static propTypes = {
    ...
    startDate: PropTypes.instanceOf(XDate),
  ...
  updateReservations(props) {
    const reservations = this.getReservations(props);
    if ((this.list && !dateutils.sameDate(props.selectedDay, this.selectedDay)) || (this.list && !this.state.loadData && reservations && reservations.scrollPosition)) {
  ...
  getReservations(props) {
    ...
    if (this.state.reservations && this.state.reservations.length) {
      ...
      while (iterator.getTime() < props.startDate.getTime()) {
    ...
    const iterator = props.startDate.clone();
    for (let i = 0; i < 500; i++) {
      const res = this.getReservationsForDay(iterator, props);
      if (res) {
        reservations = reservations.concat(res);
  ...
  render() {
    const {reservations, loading, showLoader} = this.props;
    if (!reservations || !reservations[this.props.selectedDay.toString('yyyy-MM-dd')] || loading) {
    ...
    return (
      <FlatList
        ...
        onEndReached={this.props.onReachEnd}

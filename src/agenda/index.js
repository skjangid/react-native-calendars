...
const {Text, View, Dimensions, Animated, ViewPropTypes, TouchableOpacity} = ReactNative;
...
export default class AgendaView extends Component {
  ...
  UNSAFE_componentWillReceiveProps(props) {
    ...
    if(props.selected !== this.state.selectedDay.toString('yyyy-MM-dd')) {
      this.setState({
        selectedDay: XDate(props.selected),
      });
      this._chooseDayFromCalendar(XDate(props.selected))
    }

    if (props.items && Object.keys(props.items).length && !this.state.loadData) {
      let timeOut = Object.keys(props.items).length <= 30 ? 1000 : 5000
      setTimeout(() => {
        this._chooseDayFromCalendar(XDate(this.props.selected))
      }, timeOut);
  ...
  renderReservations() {
    return (
      <ReservationsList
        ...
        onReachEnd={this.props.onReachEnd}
        onEndReachedThreshold={this.props.onEndReachedThreshold}
        onReachTop={this.props.onReachTop}
        onTopReachedThreshold={this.props.onTopReachedThreshold}
  ...
  render() {
    ...
    const monthValue = monthNames[this.state.selectedDay[0].getMonth()]
    ...
    const headerStyle = [
      this.styles.header,
      { bottom: shouldAllowDragging ? agendaHeight - 50 : agendaHeight, transform: [{ translateY: headerTranslate }] }
    ];
    ...
    const scrollPadStyle = {
      ...
      width: 80,
      height: KNOB_HEIGHT,
      top: scrollPadPosition + 46,
      left: (this.viewWidth - 80) / 2
    };

    let knob = (<View style={this.styles.knobContainer}/>);
    const divisionValue = this.viewHeight > 667 ? 25 : 18;
      ...
      knob = this.state.calendarScrollable ?
        <TouchableOpacity
          style={{
            ...
            backgroundColor: '#fff',
            //marginBottom: (this.viewHeight * divisionValue) / 667
          }} onPress={this.closeCalender.bind(this)}>
    ...
    return (
      <View testID={this.props.testID} onLayout={this.onLayout} style={[this.props.style, {flex: 1, overflow: 'hidden'}]}>
        ...
        <Animated.View style={monthDaysStyle}>
          <View style={{ flexDirection: 'row', paddingHorizontal:20,  alignItems: 'center', marginBottom: 10 }}>
            <TouchableOpacity style={{ alignItems: 'center', justifyContent: 'center', height: 28, width: 28, borderRadius: 14, backgroundColor: '#ffc096' }}>
              <Icon
                name="calendar"
                type="entypo"
                color={'#fff'}
            ...
            <Text style={{
              ...
              fontSize: 24,
              marginLeft: 8
            }}>{monthValue.substring(0, 3)} </Text>
          </View>

          <Animated.View style={weekdaysStyle}>
            {this.props.showWeekNumbers && <Text allowFontScaling={false} style={this.styles.weekday} numberOfLines={1}></Text>}
            {weekDaysNames.map((day, index) => (
              <Text allowFontScaling={false} key={day + index} style={this.styles.weekday} style={{ color: '#fff', }} numberOfLines={1}>{day[0]}</Text>
            ))}
          </Animated.View>
